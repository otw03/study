...
const { myip, urlFotmat, sendMail } = require('../helper/UtilHelper');
...
// 메일 발송이 비동기 처리를 위한 promise객체를 리턴하기 때문에 async~await문법을 적용해야 한다. 
...
router.route('/upload/single').post((req, res, next) => {
    // name 속성이 myphoto인 경우에 대한 업로드를 수행 --> multer객체가 생성되고 설정 내용이 실행됨
    // initMulter() => 객체를 리턴함
    const upload = fileHelper.initMulter().single('myphoto');

    upload(req, res, async (err) => {
        console.group('request');
        console.debug(req.file); // initMulter를 통해 req 정보를 통합한다
        console.groupEnd();
...
router.post('/send_mail2', async (req, res, next) => {
    /** 1) 프론트엔드에서 전달한 사용자 입력값 */
    let { writer_name, writer_email, receiver_name, receiver_email, subject, content } = req.body;
...
/** step10에서 추가될 내용 (반드시 모든 route처리의 맨 마지막에 위치해야 함) */

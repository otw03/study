 ...
 * @Description : 파일, 폴더 처리 관련 유틸리티 함수 구현 (업로드 기능에 유용하게 쓰임)
 * @Author: 
 */

const fs = require('fs');
const { join, extname } = require('path'); // join: 업로드명 파일이름 합치기, extname: 확장자 추출
...
class FileHelper {
    ...
    mkdirs(target, permission = '0755') {
        ...
        // target_list => [ "", "Users", "사용자이름", "Desktop"]   // Linux, Mac 의 경우(절대경로가 '/' 로 시작하기 때문)
        // target_list => [ ".", "a", "b", "c"]
        // target_list => [ "c:", "a", "b", "c"]                    // window 의 경우
        ...
        if (target_list[0].indexOf(':')  > -1) {
            target_list[0] += '/';              // 하드디스크 문자열 경로 뒤에는 '/'가 2개 붙기 때문에 미리 '/'를 1개 붙여줌
        }
        ...
        target_list.forEach((v, i) => {
            dir = join(dir, v);                 // join은 '/'를 자동으로 넣어준다
            ...
            storage: multer.diskStorage({
                ...
                destination: (req, file, callback) => {
                    /* 
                        ...
                        file = {
                            filename: 'myphoto',  // myphoto는 input태그의 name속성
            ...
            if(err instanceof multer.MulterError) {
                switch(err.code) {
                    ...
                    case "LIMIT_FILE_SIZE":
                        err.code = 500;
                        err.message = "업로드 가능한 용량을 초과했습니다.";
                        break;
                    default:
                        err.code = 500;
    ...
    async createThumbnail(file) {
        ...
        for(let i=0; i<size.length; i++) {
            ...
            const key = v + "w";
            file.thumbnail[key] = `${process.env.THUMBURL}/${thumbnail}`;

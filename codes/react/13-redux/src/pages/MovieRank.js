...
const MovieRank = memo(() => {
    ...
    // data가 변경되었을 때, 사이드 이펙트 처리하여 그래프에 적용할 데이터를 생성한다.
    ...
    return (
        <div>
             ...
             <form>
                <input type='date' className="form-control" placeholder="날짜 선택" value={targetDt} onChange={onDataChange} />
             </form>
                                ...
                                {data && data.boxOfficeResult.dailyBoxOfficeList.map((v, i) => {
                                    return (
                                        <tr key={i}>
                                            ...
                                            <td>{Number(v.salesAmt).toLocaleString()}명</td>

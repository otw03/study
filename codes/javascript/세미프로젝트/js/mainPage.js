/** 메인페이지 날씨상태, 현재기온, 지역명, 최저기온, 최고기온 강수확률 함수 mainPage() */ 
function mainPage(area, json, key, json2, json3) {
    /** 메인페이지 지도 출력함수 mainMap() */ 
    mainMap(area, key, json);
    ...
    area1.addEventListener('click', e=> {
        ...
        document.querySelector('#low').innerHTML = '최저기온' + '</br>' + minTemp(w_data, today) + '℃';  // 최저기온 함수 minTemp()
        document.querySelector('#high').innerHTML = '최고기온' + '</br>' + maxTemp(w_data, today) + '℃'; // 최고기온 함수 maxTemp()
